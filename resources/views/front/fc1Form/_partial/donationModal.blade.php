<div class="modal modal-xl fade" id="exampleModal"  aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">

                    ইতোপূর্বে গৃহীত অনুদানের বিবরণ

                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="card">
                    <div class="card-body">


                            <div class="row">

                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">উদ্দেশ্য / কার্যক্রম</label>
                                        <input type="text" name="purpose_or_activities" class="form-control" id="purpose_or_activities0"
                                        placeholder="">
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">এনজিও বিষয়ক ব্যুরো কর্তৃক অনুমোদনের স্বারক নম্বর <span class="text-danger">*</span></label>
                                        <input type="text"  name="registration_sarok_number" class="form-control" id="registration_sarok_number0"
                                        placeholder="" >
                                    </div>

                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">এনজিও বিষয়ক ব্যুরো কর্তৃক অনুমোদনের তারিখ<span class="text-danger">*</span></label>
                                        <input type="text"  name="registration_date" class="form-control" id="registration_date0"
                                        placeholder="" >
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">দাতা সংস্থার নাম</label>
                                        <input type="text" name="donor_name" class="form-control" id="donor_name0"
                                        placeholder="">
                                    </div>
                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">টাকার পরিমাণ</label>
                                        <input type="text" name="money_amount" class="form-control" id="money_amount0"
                                        placeholder="">
                                    </div>

                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">অডিট রিপোর্ট দাখিল এবং ব্যুরো কতৃক গৃহীত হয়েছে কিনা</label>
                                        <input type="text" name="audit_report" class="form-control" id="audit_report0"
                                        placeholder="">
                                    </div>

                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">সমাপ্তি প্রতিবেদন দাখিল করা হয়েছে কিনা?</label>
                                        <input type="text" name="final_report" class="form-control" id="final_report0"
                                        placeholder="">
                                    </div>

                                    <div class="col-lg-6 mb-3">
                                        <label for="" class="form-label">স্থানীয় প্রশাসনের প্রত্যয়ন পত্র দাখিল করা হয়েছে কিনা ?</label>
                                        <input type="text" name="local_certificate" class="form-control" id="local_certificate0"
                                        placeholder="">
                                    </div>


                                    <div class="col-lg-12 mb-3">
                                        <label for="" class="form-label">মন্তব্য<span class="text-danger">*</span></label>
                                        <textarea  name="comment" class="form-control" id="comment0" placeholder=""></textarea>
                                    </div>

                            </div>
                            <a id="fc1DonationAjax"  class="btn btn-registration">দাখিল করুন </a>

                    </div>
                </div>

            </div>

        </div>
    </div>
</div>
