...
@section('title')
অভিযোগ বিভাগ  | {{ trans('header.ngo_ab')}}
@endsection
                                ...
                                <div class="col-lg-6 col-sm-12">
                                    <div class="d-grid d-md-flex justify-content-end">
                                        <button type="button" class="btn btn-registration"
                                                onclick="location.href = '{{ route('complain.create') }}';">নতুন অ্যাপ্লিকেশন যোগ করুন
                                        </button>
                                    </div>
                                </div>
                            </div>
                            @if(count($complainList) == 0)
                            <div class="no_name_change">
                                <div class="d-flex justify-content-center pt-5">
                                    <img src="{{ asset('/') }}public/noresult.png" alt="" width="120" height="120">
                                </div>
                                <div class="text-center">
                                    <h5>কোনো অভিযোগ এর তালিকা নেই</h5>
                                </div>
                            </div>
                            @else
                            <div class="no_name_change pt-4">
                                <h5 class="pb-3">অভিযোগ এর তালিকা</h5>
                                ...
                                <table class="table table-bordered">
                                    ...
                                    @foreach($complainList as $key=>$allFd9List)
                                    <tr>
                                        <td>{{ $key+1 }}</td>
                                        <td>{{ $allFd9List->subject }}</td>


                                        <td>
                                            @if(empty($allFd9List->status))
...
@endif</td>
                                        <td>
                                            ...
                                            <a  href="{{ route('complain.edit',base64_encode($allFd9List->id)) }}" class="btn btn-sm btn-outline-primary"> <i class="fa fa-pencil"></i> </a>
                                            <a  href="{{ route('complain.show',base64_encode($allFd9List->id)) }}" class="btn btn-sm btn-outline-success"> <i class="fa fa-eye"></i> </a>
                                            <button type="button" onclick="deleteTag({{ $allFd9List->id}})" class="btn btn-sm btn-outline-danger"><i
                                                class="bi bi-trash"></i></button>

                                                <form id="delete-form-{{ $allFd9List->id }}" action="{{ route('complain.destroy',$allFd9List->id) }}" method="POST" style="display: none;">
